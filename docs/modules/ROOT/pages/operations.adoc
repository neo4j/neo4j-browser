...
_neo4j+s://_::
When connection to a cluster or a single instance and routing are handled by the driver.
Use this for connection to Neo4j Aura or if you connect to a cluster.
There are two versions of this URI; connection encrypted with TLS `neo4j+s://` and no TLS `neo4j://`.

_bolt+s://_::
To connect directly to a single instance without any routing by the driver.
Use this for connecting directly to a specific cluster member.
There are two versions of this URI; connection encrypted with TLS `bolt+s://` and no `TLS bolt://`.
...
You can set several parameters with the `:params` command, this will also clear all currently set parameters.

[NOTE]
====
Integers will be set to float with this style.
====
...
The parameters will not be saved when you close the browser.
You can save a `:params` command as a favorite to be able to quickly populate parameters again if you got access to Browser Sync or if you are using Neo4j Desktop.
