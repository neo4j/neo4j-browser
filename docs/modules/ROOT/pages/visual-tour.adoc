...
All favorites are stored in your local browser storage and as such are centric to the user/browser who has recorded those favorites.
...
* *Common Procedures* contains a few built-in procedures, two of which list all available functions and procedures.

[[saved-files]]
=== Project Files

The *Project files* tab is Neo4j Desktop specific.
Project files allows you to save queries, guides, and other scripts, as cypher files.
...
| `:history`
| Display the most recent executed commands.
...
| `maxHistory`
| `30`
| The maximum number of recent executed queries kept in Browser.
When reached, old entries gets retired.
The command history is persisted across Browser restarts.
| yes
...
| `initCmd`
| `:play start`
| The initial command that will be executed when connected to the graph.
| yes

| `playImplicitInitCommands`
| `true`
| TODO: Commands that will be executed when connected to the graph.
| no
...
| `maxFieldItems`
| `500`
| Maximum number of items per field.
| no
...
| `codeFontLigatures`
| `true`
| Allows font ligatures for the Cypher editor bar and Cypher snippets.
| yes

| `useBoltRouting`
| `false`
| Whether to use Bolt routing.
The Bolt+routing is the feature on Neo4j drivers to discover the member of a cluster (drivers have a load-balancing feature).
Therefore, this feature is only useful if you have a causal cluster.
| no
...
| `true`
| Allows you to write and edit multi-line queries (use semicolon to separate statements).
| yes
...
====
The example shows how to change the maximum number of neighbours for a node.
[source, cypher]
----
...
----
Once making the change, rerunning `:config` should report that `maxFrames` has been set to `10` and `theme` is defined to `outline`.
The Browser Settings Icon should also reflect this change.
====
...
Separate multiple commands with semi-colon.
...
Each reusable frame maintains their own local history of commands and updates the main one in the Cypher editor, should you need to instantiate a new result frame from there.
...
* Graph - display the result as nodes and relationships, and allows xref:operations.adoc#styling[styling] to be configured.
...
For more details on the different ways to view data in Neo4j Browser, see xref:operations.adoc#results[Visualizing results].
