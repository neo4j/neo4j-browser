...
* *Node Counts* and *Relationship counts*  --  Displayed in parentheses after the *. Note that the counts stop updating automatically on big graphs for users who don't have access to all data, see link:https://neo4j.com/docs/browser-manual/current/operations/browser-settings/#rbac-note[A note on Role-Based Access Control^].
