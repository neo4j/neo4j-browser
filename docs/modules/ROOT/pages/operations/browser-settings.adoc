...
To change the configuration settings locally in Neo4j Browser, you can either use the tabs in the Browser Settings drawer or, in the Editor, type the command `:config` together with the setting and the new value.

.Adjust settings in the Browser UI
image:settings.png[width=40%]

.Adjust an individual setting using `:confiug`
====
