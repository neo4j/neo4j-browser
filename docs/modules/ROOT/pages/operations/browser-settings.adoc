...
However, if link:https://neo4j.com/docs/operations-manual/current/authentication-authorization/access-control/[role-based access control^] is configured, Neo4j counts the nodes and relationships the current user has access to on-demand, which is computationally expensive and thus potentially time-consuming. 
