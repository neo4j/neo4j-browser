...
SSO has been tested on Neo4j 4.2.x, 4.3.x, and 4.4.x deployments.
...
[IMPORTANT]
====
Security information should always be exchanged use encrypted transport, and therefore HTTPS should be used.
Mixed HTTP/HTTPS flows for single sign-on are not supported.
====

Once your SSO provider is configured, you need to update the following in the link:https://neo4j.com/docs/operations-manual/current/configuration/neo4j-conf/#neo4j-conf[`neo4j.conf`^] file accordingly:

[source, properties]
----
dbms.security.auth_enabled=true
dbms.security.authentication_providers=<provider>,native
dbms.security.authorization_providers=<provider>,native
----

The placeholder `<provider>` should be replaced with a name representing your SSO provider.
